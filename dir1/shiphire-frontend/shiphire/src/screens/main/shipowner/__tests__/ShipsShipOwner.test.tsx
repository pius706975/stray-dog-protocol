...
const mockGetOwnerShipsResponse = {
    status: 'success',
    data: [
        {
            size: {
            ...
            category: '65a4d4184fa531643fc8cdcb',
            tags: [],
            shipDocuments: [
                {
        ...
        },
        {
            ...
            category: '65a4d4184fa531643fc8cdcd',
            tags: [],
            shipDocuments: [
                {
        ...
        },
        {
            ...
            category: '65a4d4184fa531643fc8cdcf',
            tags: [],
            shipDocuments: [
                {
...
const mockGetShipOwnerDataCompanyRejectedResponse = {
    status: 'success',
    data: {
        company: {
            isRejected: true,
...
const mockGetShipOwnerDataCompanyNotVerifResponse = {
    status: 'success',
    data: {
        company: {
            name: 'Fauzan Company',
...
describe('Testing Ships Screen', () => {
    describe('Snapshot Testing', () => {
        it('should render ships screen correctly', () => {
            const shipsScreen = render(shipsMockComponent);
            expect(shipsScreen).toMatchSnapshot();
        });
    });
    describe('Unit Testing', () => {
        it('should show text ships in header', () => {
            const { getByText } = render(shipsMockComponent);
            const textShips = getByText('placeholderSearch');
            expect(textShips).toBeTruthy();
        });
        it('should render ships list correctly', () => {
            const { getByTestId } = render(shipsMockComponent);
            mockAdapter
                .onGet('/ship-owner/get-ships')
                .reply(200, mockGetOwnerShipsResponse);
            const shipsList = getByTestId('shipListView');
            expect(shipsList).toBeDefined();
        });
        ...
        it('should handle succes when get ship owner company success', async () => {
            const { getByTestId } = render(shipsMockComponent);
            mockAdapter
                .onGet('/ship-owner/get-ship-owner-data')
                .reply(200, mockGetShipOwnerDataResponse);
            expect(getByTestId('shipListView')).toBeDefined();
        });
        it('should handle when company is not verified', async () => {
            const { getByTestId } = render(shipsMockComponent);
            const addShipButton = getByTestId('addShipButton');
            fireEvent.press(addShipButton);
            mockAdapter.onGet('/ship-owner/get-ship-owner-data').reply(
                200,

                mockGetShipOwnerDataCompanyNotVerifResponse,
            );
            await waitFor(() => {
                const modalState = store.getState().modal;
                ...
                expect(modalState).toEqual({
                    visible: true,
                    status: 'info',
                    text: 'infoCompanyNotVerif',
                });

                jest.advanceTimersByTime(4000);
            });
        });
        it('should handle when company is rejected', async () => {
            mockAdapter
                .onGet('/ship-owner/get-ships')
                .reply(200, mockGetOwnerShipsResponse);
            mockAdapter
                .onGet('/ship-owner/get-ship-owner-data')
                .reply(200, mockGetShipOwnerDataCompanyRejectedResponse);
            const { getByTestId, rerender } = render(shipsMockComponent);
            const addShipButton = getByTestId('addShipButton');
        ...
        // it('should handle add ship button correctly when press add icon', async () => {
        //     const { getByTestId } = render(shipsMockComponent);
        //     const addShipButton = getByTestId('addShipButton');
        //     fireEvent.press(addShipButton);
        //     mockAdapter
